...
function initializeWorkspaceState(context) {
    ...
    const currentModel = workspaceState.get("ralph.lastModel");
    if (!currentModel) {
        workspaceState.update("ralph.lastModel", defaultModel);
    }
    const currentModelId = workspaceState.get("ralph.lastModelId");
    const effectiveModel = currentModel ?? defaultModel;
    if (currentModelId === undefined && effectiveModel === protobuf_1.DEFAULT_MODEL_NAME) {
        workspaceState.update("ralph.lastModelId", protobuf_1.DEFAULT_MODEL_ID);
    }
    ...
    if (!currentMode || currentMode === "Building") {
        workspaceState.update("ralph.lastMode", "Fast");
    }
}
