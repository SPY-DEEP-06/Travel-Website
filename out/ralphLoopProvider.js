...
class RalphLoopProvider {
    ...
    getRootItems() {
        ...
        if (state.modelsLoading) {
            return [
                new RalphLoopItem("models-loading", "Loading models", "Waiting for Antigravity model discovery", vscode.TreeItemCollapsibleState.None, new vscode.ThemeIcon("loading~spin")),
            ];
        }
        return [
    ...
    getConfigItems() {
        ...
        const modelFallbackSelections = (0, modelCatalog_1.buildModelFallbackSelections)(modelFallbackBucketOrder, workspaceState.get("ralph.modelFallbackSelections", []));
        const visibleFallbackSelections = (0, modelCatalog_1.getFallbackSelectionsForPrimary)(modelFallbackSelections, (0, modelCatalog_1.getModelQuotaBucket)(model));
        ...
        const items = [
            ...
            new RalphLoopItem("config-model", "Primary Model", model, vscode.TreeItemCollapsibleState.None, new vscode.ThemeIcon("hubot")),
            new RalphLoopItem("config-model-fallback-priority", "Fallback Chain", `Rate limit: ${(0, modelCatalog_1.formatModelFallbackSelections)(visibleFallbackSelections)}`, vscode.TreeItemCollapsibleState.None, new vscode.ThemeIcon("list-ordered")),
        ...
        const commandById = {
            ...
            "config-model": {
                command: "ralph.setConfigModel",
                title: "Set Primary Model",
            },
            "config-model-fallback-priority": {
                command: "ralph.setConfigModelFallbackPriority",
