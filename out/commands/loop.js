...
async function startRalphLoop(context) {
    ...
    try {
        ...
        if (config.modelFallbackSelections.length > 0) {
            state.progressLogger?.info(`Fallback Chain: ${(0, modelCatalog_1.formatModelFallbackSelections)((0, modelCatalog_1.getFallbackSelectionsForPrimary)(config.modelFallbackSelections, (0, modelCatalog_1.getModelQuotaBucket)(config.model)))}`, "Config");
        }
