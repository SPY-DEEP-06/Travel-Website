...
function extractAntigravityFromProcessWindows(workspacePath) {
    ...
    for (const proc of processes) {
        ...
        if (isLanguageServer && isAntigravity) {
            ...
            const extensionServerCsrfMatch = cmdLine.match(/--extension_server_csrf_token\s+([a-f0-9-]+)/i);
            const extensionServerCsrfToken = extensionServerCsrfMatch
                ? extensionServerCsrfMatch[1]
                : undefined;
...
function extractAntigravityFromProcessUnix(workspacePath) {
    ...
    for (const line of lines) {
        ...
        if (isLanguageServer && isAntigravity) {
            ...
            const extensionServerPort = portMatch ? parseInt(portMatch[1], 10) : undefined;
            const extensionServerCsrfMatch = line.match(/--extension_server_csrf_token\s+([a-f0-9-]+)/i);
            const extensionServerCsrfToken = extensionServerCsrfMatch
                ? extensionServerCsrfMatch[1]
                : undefined;
...
async function discoverAntigravityPort(pid, workspacePath) {
    ...
    // Antigravity already exposes its extension server port on the language server
    // command line. Probe that first before relying on OS port enumeration.
    if (processInfo?.extensionServerPort) {
        const isKnownPortGrpc = await probeGrpcPort(processInfo.extensionServerPort, processInfo.csrfToken);
        if (isKnownPortGrpc) {
            return processInfo.extensionServerPort;
        }
    }
    let listeningPorts = [];
    if (isWindows) {
